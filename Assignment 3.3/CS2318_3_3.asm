...
# $t4: reply
# $t5: mean	#should be callee saved to prevent clobbering? from call to LTGTmean
...
#                         ProcArrays(mean, a1, a2, a3, used1, &used2, &used3);
					...
					addi $a2, $sp, 240			#a2
					addi $a3, $sp, 288			#a3
					lw $t0, 188($sp)				#contents of used1 to $t0
					sw $t0, 16($sp)
					addi $t0, $sp, 184			#address of used2
					sw $t0, 20($sp)
					addi $t0, $sp, 180			#address of used3
					sw $t0, 24($sp)
...
#################
# $s0: total
# $t0: target			#Why no stack space for target var? Bc no need to save it. (Short lived)
# $t1: hopPtr1
# $t9: endPtr1
...
#                       *endPtr1;
					...
					sw $a0, 0($fp)				#Check when to make sure to save incoming 
					sw $a1, 4($fp)				#arguments to callers frame as received
					sw $a2, 8($fp)				#
					#sw $a3, 12($fp)			#Don't save $a3 bc it's not clobbered
					
					sw $s0, 16($sp)				#save callee-saved $s0
					
					# BODY:
#                   *used2Ptr = 0;
					lw $v1, 16($fp)				#load addr of used2Ptr
					sw $0, 0($v1)				#store 0 to contents of used2Ptr
#                   *used3Ptr = 0;
					lw $v1, 20($fp)
					sw $0, 0($v1)
#                   total = 0;
...
#                   hopPtr1 = a1;
					move $t1, $a0
#                   endPtr1 = a1 + used1;
					sll $v1, $a3, 2				#$v1 = used1 * 4
					add $t9, $a0, $v1			#$endPtr1 = a1 + $v1
#                   goto FTest_PA1223;
					j FTest_PA1223
begF_PA1223:
#                      target = *hopPtr1;
...
#                      total += target;
					add $s0, $s0, $t0
#                      if (target % 2 == 0) goto else_PA1223; (even number --> goto else_PA)
					andi $v1, $t0, 1				#Bit operation. 1 if odd. 0 if even.
					beqz $v1, else_PA1223
begI_PA1223:
...
#                   return total/used1;
					div $v0, $s0, $a3			#preserve $a3. (AuxO and AuxE don't clobber $a3)
					
					# EPILOG:
					lw $s0, 16($sp)				#restore callee-saved $s0
...
#                   hopPtr3 = a3 + *used3Ptr - 1;
					lw $v1, 0($a1)				#load contents of used3ptr to $v1
					addi $v1, $v1, -1			#*used3Ptr - 1
					sll $v1, $v1, 2				#multiply by 4
					add $t1, $a0, $v1			#hopPtr3 = a3 + ...^
...
begI_PA1223AO:
#                         *(hopPtr3 + 1) = *hopPtr3;
					lw $v1, 0($t1)				#load contents of hopPtr3 into $v1
					sw $v1, 4($t1)				#store $v1 into *(hopPtr3+1)
...
#                       *endPtr2;
					...
					sw $s0, 0($sp)					#save $s0 from caller (popArr1223)
					
					# BODY:
#                   hopPtr2 = a2;
					move $t1, $a0
#                   endPtr2 = a2 + *used2Ptr;
					lw $v1, 0($a1)					#load contents of used2ptr
					sll $v1, $v1, 2					#multiply by 4
					add $t9, $a0, $v1				#add a2[] and ^
...
begW2_PA1223AE:
#                            *hopPtr21 = *(hopPtr21 - 1);
					lw $v1, -4($s0)					#load contents of (hopPtr21-1) to $v1
					sw $v1, 0($s0)					#store $v1 to hopPtr21 contents
#                            --hopPtr21;
					addi $s0, $s0, -4
WTest2_PA1223AE:
...
#                   ++(*used2Ptr);
					...
					# EPILOG:
					lw $s0, 0($sp)					#Restore $s0 for caller (popArr1223)
...
###############################################################################
					...
					sw $a0, 0($fp)				#Check when to make sure to save incoming 
					sw $a1, 4($fp)				#arguments to callers frame as received
					sw $a2, 8($fp)				#Perhaps save these when being clobbered and need
					sw $a3, 12($fp)				#for later use?					
...
#                   goto WTest1_MC2321;
					...
					lw $v1, 0($a0)
					sll $v1, $v1, 2
					add $t8, $a2, $v1			#endPtr2 = a2 + *used2ptr
					
					lw $v1, 16($sp)				#load addr of used3ptr
					lw $t0, 0($v1)				#load contents of used3ptr to $t0
					sll $t0, $t0, 2				#multiply used3 by 4
					add $t9, $a3, $t0			#$t9 = a3 + *used3ptr
...
#                         *hopPtr2 = target;
					sw $t5, 0($t2)	
#                         ++(*used2Ptr);
					lw $v1, 20($sp)				#load addr of used2ptr
					lw $t0, 0($v1)				#load value at used2ptr
					addi $t0, $t0, 1				#add 1 to value
					sw $t0, 0($v1)				#store new value to used2ptr
#                         ++hopPtr2;
					addi $t2, $t2, 4
